...
Stationarity (weak) and isotropy (strong) assumptions are best tackled theoretically by the less theoretically-inclined. Are we missing any linear fixed effects or spatial variance terms that would affect the spatial patterns across the pixels/points (i.e., stationarity) or across the whole study area in larger space (i.e. isotropy)? Maybe georichness is a strong explainer of species richness in the Kilpisjärvi area that we do not account for via basic topography? Ordination methods (e.g., `gllvm`) can offer a nice way to examine the unmodelled structures.

Predictive scoring is a complex topic if you're interested prediction, and `inlabru` has a nice theoretical description and links to another package, `scoringRules` that can provide more help if you consult [the official article](https://inlabru-org.github.io/inlabru/articles/prediction_scores.html). These generally require some testing set (data that we do not use to fit the model but generated in the same way) or at least a theoretically generated comparison dataset. Both of these are too much work for this exercise. In my experience, CPRS is often treated as a "gold standard" for predictive metrics.
