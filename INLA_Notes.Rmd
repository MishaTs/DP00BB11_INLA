...
elevT <- crop(terra::rast(elevRL, "epsg:4326"), 
              ...
              meshBounds)
# de-mean elevation to match vignette example
elev <- elevT - mean(terra::values(elevT), na.rm = TRUE)

# helper function because the example default method doesn't work well
f.elev <- function(where) {
  # Extract values from terra object using special inlabru function
  v <- eval_spatial(elev, where, layer = names(elev))
  v
}

# define the model
formRast <- species_rich ~ elev(f.elev(.data.), model = "linear") +
  field(geometry, model = matern2D) + Intercept(1)
# run the model
fitGeoRast <- bru(formRast, kjGeo, family = "tweedie")
```
