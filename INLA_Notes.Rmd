...
A good rule of thumb is to **always prefer default priors unless you have a reason not to**. For advanced model tweaks though, it can get quite interesting. INLA priors often work through **hyperparameters**: e.g., $\mu$ and $\sigma$ in Gaussian distribution. We will show hyperparameter checks at the end of our worked example to demonstrate how you can interpret if, for example, you're using the correct distribution family (e.g., Gaussian, Poisson) based on posterior hyperparameters.
...
We'll use the Kilpisjärvi data from previous examples. We have spatial coordinates in EUREF-FIN and WGS84, species richness, individual species' covers (as a %), trait information, and some climate variables. We'll use the WGS84 coordinates along with trait and climate covariates to model species richness.
...
INLA supports an dizzying amount of distribution families that aren't well [documented](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/). For our case, [Gaussians](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/gaussian.pdf) are a decent baseline choice but cannot be constrained to positive values only. [Poisson](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/poisson.pdf) is a common alternative (especially [non-zero Poisson](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/nzpoisson.pdf)) though dispersion is always a concern. [Negative binomial distributions](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/nbinomial.pdf) can present another good option. 
...
In it is most of the non-spatial information we need. Contrary to what you might think, the only *truly spatial* part of our `inlabru` model is the `field` itself. Everything else functions normally via regression rules. I demonstrate how to interpret the intercept and one coefficient:
...
Dealing with the spatial field is a bit more complicated and is best handled using the `predict` function. This is relatively new: example articles and vignettes are slowly showing the ways it can be used to replace traditional model outputs for more robust estimation. Predictions are especially useful at understanding the model's spatial behaviour. We show the mean model prediction when all covariates are at their average values (including the spatially variant field and the fixed intercept).
...
We can also look at the underlying parameters driving spatial field behaviour, mainly the range and variance. These can be constrained with priors using `inla.spde2.pcmatern` if they look unsatisfactory (e.g., if you want a more constrained variance or broader range). The distributions' shapes look the same because of the underlying SPDE model architecture.
...
Plotting against richness, it looks like our spatial model should be refined. The gap around 0 isn't an issue and [an effect of the log transformation](https://groups.google.com/g/r-inla-discussion-group/c/572O2Nrev5Y/m/QaxKIiibFQAJ). But we both underestimate higher richness and overestimate lower richness. This isn't that shocking: a simple model will pull everything towards the mean value. We don't have any 0 values in the data; if we did, the strong negative residuals for low richness might motivate a zero-inflated distribution. Try out a residual vs fitted plot if you'd like to see how a cruder metric compares.
...
Finally, it's always good practice to look at at least the distribution posterior hyperparameters: was Tweedie a good choice? First we see our dispersion parameter quite narrowly around its mean `r signif(fitGeo$summary.hyperpar$mean[2], 2)`. So Poisson would have  performed poorly. The power parameter behaves similarly around its mean `r signif(fitGeo$summary.hyperpar$mean[1], 2)`: the Tweedie ends up neither a full gamma or quasi-Poisson distribution, and simplifying it to one of its constituent distributions is probably a poor idea. So we do a decent job specifying an effective mean-variance relationship in our underling distribution. These numbers are all found in the first `summary()` call but can be directly accessed using `fitGeo$summary.hyperpar`. 
